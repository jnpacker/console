...
// Local copy that uses modified NodePoolsTable
...
const getNodePoolsStatus = (
  ...
  const nodePoolMap = nodePools.reduce<{
    [key: string]: { agents: AgentK8sResource[]; status: any };
  }>((acc, np) => {
        ...
        {isExpanded && (
          <StackItem className="ai-progress-item__body">
            <NodePoolsTable
              ...
              vmwareTemplates={rest.vmwareTemplates}
              onRemoveNodePool={rest.onRemoveNodePool}
              onUpdateNodePool={rest.onUpdateNodePool}
              onAddNodePool={rest.onAddNodePool}
              supportedVersionsCM={rest.supportedVersionsCM}
            />
          </StackItem>
        )}
      </Stack>
