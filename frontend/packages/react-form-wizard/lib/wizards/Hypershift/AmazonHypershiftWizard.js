...
import { Section, WizSelect, Step, WizardPage, WizArrayInput, WizCheckbox, WizDetailsHidden, WizItemSelector, WizMultiSelect, WizNumberInput, WizSingleSelect, WizTextDetail, WizTextInput, } from '../../src';
...
export function AmazonHypershiftWizard(props) {
    const history = useHistory();
    const clusterSets = props.clusterSets.map((clusterSet) => clusterSet.metadata?.name);
    return (_jsxs(WizardPage, { title: "Create cluster", breadcrumb: [{ label: 'Managed clusters', to: '.' }], onSubmit: () => Promise.resolve(undefined), onCancel: () => history.push('./?route=wizards'), defaultData: [
            defaultData,
            {
                apiVersion: 'v1',
                kind: 'Secret',
        ...
        ], children: [_jsx(Step, { label: "Prerequsites", id: "prerequsites", children: _jsx(Section, { label: "Prerequsites", autohide: false, children: _jsx(WizDetailsHidden, { children: _jsxs(Stack, { hasGutter: true, children: [_jsx(List, { children: _jsx(ListItem, { icon: _jsx(Icon, { status: "success", children: _jsx(CheckIcon, {}) }), children: _jsxs(Stack, { style: { width: '100%' }, hasGutter: true, children: [_jsx("div", { children: "The hypershift-preview component is enabled on the multi-cluster engine." }), _jsx(CodeBlock, { children: `oc get mce multiclusterengine-sample -ojsonpath="{.spec.overrides.components[?(@.name=='hypershift-preview')].enabled}"` })] }) }) }), _jsx(List, { children: _jsx(ListItem, { icon: _jsx(Icon, { status: "success", children: _jsx(CheckIcon, {}) }), children: _jsxs(Stack, { style: { width: '100%' }, hasGutter: true, children: [_jsx("div", { children: "Managed cluster addon for hypershift is configured." }), _jsx(CodeBlock, { children: `oc get mca hypershift-addon -n local-cluster` })] }) }) }), _jsx(List, { children: _jsx(ListItem, { icon: _jsx(Icon, { status: "danger", children: _jsx(CloseIcon, {}) }), children: _jsxs(Stack, { style: { width: '100%' }, hasGutter: true, children: [_jsx("div", { children: "An OIDC s3 credentials secret for the HyperShift operator to use to access a public s3 bucket" }), _jsx(CodeBlock, { children: `oc get secret hypershift-operator-oidc-provider-s3-credentials -n local-cluster` }), _jsxs("div", { children: ["See Getting started in the", ' ', _jsx("a", { href: "https://hypershift-docs.netlify.app/getting-started", target: "_blank", rel: "noreferrer", children: "HyperShift" }), ' ', "documentation for more information about the secret."] }), _jsx(CodeBlock, { children: "aws s3api create-bucket --acl public-read --bucket your-bucket-name" }), _jsx("div", { children: "The following example shows how to create the secret:" }), _jsx(CodeBlock, { children: `oc create secret generic hypershift-operator-oidc-provider-s3-credentials --from-file=credentials=$HOME/.aws/credentials --from-literal=bucket=<your-bucket-name> --from-literal=region=<region> -n local-cluster` })] }) }) })] }) }) }) }), _jsx(Step, { label: "Cluster details", id: "cluster-details", children: _jsx(WizItemSelector, { selectKey: "kind", selectValue: "HypershiftDeployment", children: _jsxs(Section, { label: "Cluster details", prompt: "Enter the cluster details", children: [_jsx(Sync, { kind: "HypershiftDeployment", path: "metadata.name", targetPath: "metadata.name" }), _jsx(WizTextInput, { path: "metadata.name", label: "Cluster name", placeholder: "Enter the cluster name", required: true, labelHelp: "The unique name of your cluster. The value must be a string that contains lowercase alphanumeric values, such as dev. Cannot be changed after creation." }), _jsx(WizSelect, { path: "spec.hostingNamespace", label: "Cluster set", placeholder: "Select a cluster set", labelHelp: "A cluster set is a group of clusters that can be targeted as a group.", options: clusterSets, required: true }), _jsx(WizSelect, { path: "spec.infrastructure.release.image", label: "Release image", placeholder: "Select a release image", options: ['default'], required: true }), _jsx(WizCheckbox, { path: "spec.hostedClusterSpec.fips", label: "FIPS", helperText: "Use the Federal Information Processing Standards (FIPS) modules provided with Red Hat Enterprise Linux CoreOS instead of the default Kubernetes cryptography suite." })] }) }) }), _jsx(Step, { label: "Credentials", id: "credentials", children: _jsx(WizItemSelector, { selectKey: "kind", selectValue: "HypershiftDeployment", children: _jsxs(Section, { label: "Credentials", children: [_jsx(WizSingleSelect, { path: "spec.infrastructure.cloudProvider.name", label: "Infrastructure credentials", placeholder: "Select the credentials for the infrastructure", options: ['default'], required: true }), _jsx(WizTextInput, { path: "spec.hostedClusterSpec.dns.baseDomain", label: "Base DNS domain", labelHelp: "The base domain of your provider, which is used to create routes to your OpenShift Container Platform cluster components. It is configured in your cloud provider's DNS as a Start Of Authority record (SOA). Cannot be changed after creation.", placeholder: "Enter the Base DNS domain", helperText: "Defaults to the base DNS domain from the credentials." }), _jsx(WizSelect, { path: "spec.infrastructure.release.platform.image", label: "Release image", placeholder: "Select a release image", options: ['default'] }), _jsx(WizCheckbox, { path: "spec.hostedClusterSpec.fips", label: "FIPS" })] }) }) }), _jsx(Step, { label: "Worker pools", id: "worker-pools", children: _jsx(WizItemSelector, { selectKey: "kind", selectValue: "HypershiftDeployment", children: _jsxs(Section, { label: "Worker pools", prompt: "Enter the worker pools", description: "One or more worker nodes will be created to run the container workloads in this cluster.", children: [_jsx(WizSelect, { path: "spec.infrastructure.platform.aws.region", label: "Region", options: Object.keys(awsRegions), required: true, labelHelp: "The AWS region where the installation program creates your cluster resources. You can select zones within the region for your control plane and worker pools." }), _jsx(WizMultiSelect, { path: "spec.infrastructure.platform.aws.zones", label: "Zones", options: awsRegions['us-east-1'] }), _jsxs(WizArrayInput, { path: "spec.nodePools", label: "Worker pools", placeholder: "Add worker pool", collapsedContent: _jsx(WizTextDetail, { path: "name", placeholder: "Expand to edit the worker pool details" }), newValue: newNodePool, children: [_jsx(WizTextInput, { path: "name", label: "Pool name", required: true }), _jsx(WizSelect, { path: "spec.platform.aws.instanceType", label: "Instance type", options: ['default'], required: true })] })] }) }) }), _jsx(Step, { label: "Autoscaling", id: "autoscaling", children: _jsx(WizItemSelector, { selectKey: "kind", selectValue: "HypershiftDeployment", children: _jsxs(Section, { label: "Autoscaling", children: [_jsx(WizNumberInput, { path: "spec.hostedClusterSpec.autoscaling.maxNodesTotal", label: "Max nodes total", labelHelp: "MaxNodesTotal is the maximum allowable number of nodes across all NodePools for a HostedCluster. The autoscaler will not grow the cluster beyond this number." }), _jsx(WizTextInput, { path: "spec.hostedClusterSpec.autoscaling.maxNodeProvisionTime", label: "Max node provision time", labelHelp: "MaxNodeProvisionTime is the maximum time to wait for node provisioning before considering the provisioning to be unsuccessful, expressed as a Go duration string. The default is 15 minutes." }), _jsx(WizNumberInput, { path: "spec.hostedClusterSpec.autoscaling.maxPodGracePeriod", label: "Max pod grace period", labelHelp: "MaxPodGracePeriod is the maximum seconds to wait for graceful pod termination before scaling down a NodePool. The default is 600 seconds.", helperText: "The period in seconds." }), _jsx(WizNumberInput, { path: "spec.hostedClusterSpec.autoscaling.podPriorityThreshold", label: "Pod priority threshold", labelHelp: "PodPriorityThreshold enables users to schedule 'best-effort' pods, which shouldn't trigger autoscaler actions, but only run when there are spare resources available. The default is -10.", min: -9999 })] }) }) }), _jsx(Step, { label: "Networking", id: "networking", children: _jsx(WizItemSelector, { selectKey: "kind", selectValue: "HypershiftDeployment", children: _jsxs(Section, { label: "Networking", prompt: "Enter networking options", description: "Configure network access for your cluster.", children: [_jsx(WizSingleSelect, { path: "spec.hostedClusterSpec.networking.networkType", label: "Network type", options: ['OpenShiftSDN', 'OVNKubernetes'], required: true }), _jsx(WizTextInput, { path: "spec.hostedClusterSpec.networking.podCIDR", label: "Pod CIDR", required: true }), _jsx(WizTextInput, { path: "spec.hostedClusterSpec.networking.serviceCIDR", label: "Service CIDR", required: true }), _jsx(WizTextInput, { path: "spec.hostedClusterSpec.networking.machineCIDR", label: "Machine CIDR", required: true })] }) }) })] }));
}
export function ControlPlaneStep() {
    return (_jsxs(Section, { label: "Control plane", prompt: "Enter the control plane details", description: "Three control plane nodes will be created to control this cluster unless single node is enabled, in which case there will only be one control plane node.", children: [_jsx(WizMultiSelect, { path: "zones", label: "Zones", options: awsRegions['us-east-1'] }), _jsx(WizSelect, { path: "instanceType", label: "Instance type", placeholder: "Select the instance type", options: AWSmasterInstanceTypes.map((instanceType) => ({
                    value: instanceType.value,
                    label: instanceType.description,
                })), required: true }), _jsx(WizSelect, { path: "rootStorage", label: "Root storage (GiB)", options: ['default'] })] }));
}
...
//# sourceMappingURL=AmazonHypershiftWizard.js.map